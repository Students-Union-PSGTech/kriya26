...
const ProfileHeader = ({ user, onLogout, isLoggingOut, onProfileUpdate }) => {
    ...
    const [pendingAvatar, setPendingAvatar] = React.useState(avatarNumber);
    const [editAvatar, setEditAvatar] = React.useState(avatarNumber);
    ...
    const saveAvatar = (num) => {
        localStorage.setItem(AVATAR_STORAGE_KEY, String(num));
        setAvatarNumber(num);
        setEditAvatar(num);
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            ...
            // Save avatar choice
            localStorage.setItem(AVATAR_STORAGE_KEY, String(editAvatar));
            setAvatarNumber(editAvatar);
    ...
    // --- Avatar Grid Component (reused in both modals) ---
    const AvatarGrid = ({ selected, onSelect, size = "w-14 h-14" }) => (
        <div className="grid grid-cols-4 gap-3">
            {Array.from({ length: TOTAL_AVATARS }, (_, i) => i + 1).map((num) => (
                <button
                    ...
                    className={`relative ${size} rounded-full overflow-hidden border-2 transition-all duration-200 ${selected === num
                ...
                >
                    ...
                    {selected === num && (
                        <div className="absolute inset-0 bg-blue-500/20 flex items-center justify-center">
                            <IoCheckmarkCircle className="text-blue-400 text-xl drop-shadow-lg" />
                        </div>
                    )}
                </button>
            ))}
        ...
        <div className="relative w-full overflow-hidden rounded-xl bg-white/5 backdrop-blur-md border border-white/10 p-6 md:p-8">
            <div className="flex flex-col md:flex-row items-center gap-6 md:gap-8">
                ...
                >
                    ...
                    {/* Pencil icon overlay */}
                    <div className="absolute bottom-0 right-0 bg-blue-500 rounded-full p-1.5 border-2 border-black shadow-lg opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-200">
                        <TiEdit className="text-white text-sm" />
                    </div>
                </div>
            ...
            {/* ===== Avatar Chooser Modal ===== */}
            {isAvatarModalOpen && typeof window !== 'undefined' && ReactDOM.createPortal(
                <div
                    className="fixed inset-0 z-9999 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md"
                    onClick={() => setIsAvatarModalOpen(false)}
                >
                    <div
                        className="relative w-full max-w-xl bg-black/90 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-2xl"
                    ...
                    >
                        ...
                        <div className="border-t border-white/10 p-6 pt-4 bg-black/50 backdrop-blur-md">
                            <div className="flex gap-3">
                                <button
                                    onClick={() => setIsAvatarModalOpen(false)}
                                    className="flex-1 px-6 py-3 bg-white/5 border border-white/10 text-white rounded-lg font-general text-sm uppercase tracking-wider hover:bg-white/10 transition-all"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={() => saveAvatar(pendingAvatar)}
                                    className="flex-1 px-6 py-3 bg-blue-500 hover:bg-blue-600 border border-blue-400 text-white rounded-lg font-general text-sm uppercase tracking-wider transition-all shadow-lg shadow-blue-500/20"
                                >
                                    Save
                                </button>
                            </div>
                ...
                <div className="fixed inset-0 z-9999 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
                    <div className="relative w-full max-w-2xl max-h-[90vh] flex flex-col bg-black/90 backdrop-blur-md border border-white/10 rounded-xl overflow-hidden shadow-2xl">
                        ...
                        <div className="flex-1 overflow-y-auto px-6 md:px-8 pb-4">
                            ...
                            {/* Avatar Picker */}
                            <div className="mb-6">
                                <label className="block text-[10px] uppercase tracking-wider text-blue-500 font-bold mb-3">
                                    Profile Picture
                                </label>
                                <AvatarGrid selected={editAvatar} onSelect={setEditAvatar} />
                            </div>
